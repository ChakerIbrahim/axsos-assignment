...
public class BookController {
    ...
    // GET /bookmarket : the Book Lender Dashboard
    ...
    public String bookmarket(HttpSession session, Model model) {
        ...
        model.addAttribute("availableBooks", bookServ.availableBooks());
        model.addAttribute("borrowedBooks", bookServ.borrowedBooks(user));
        return "bookmarket.jsp";
    }

    // GET /bookmarket/{id}/borrow : book moves top table -> bottom table
    ...
    public String borrow(@PathVariable("id") Long id, HttpSession session) {
        ...
        bookServ.borrowBook(book, user);
        return "redirect:/bookmarket";
    }

    // GET /bookmarket/{id}/return : book moves back to the top table
    ...
    public String returnBook(@PathVariable("id") Long id, HttpSession session) {
        ...
        if (book.getBorrower() != null
                && book.getBorrower().getId().equals(userId)) {
            bookServ.returnBook(book);
        }
        return "redirect:/bookmarket";
    }
    ...
    public String updateBook(@PathVariable("id") Long id,
        ...
        // Fetch the ORIGINAL book and copy its relationships
        // onto the submitted object before saving:
        Book original = bookServ.findBook(id);

        book.setId(id);                             // keep the id -> save() UPDATEs
        book.setUser(original.getUser());           // keep the original poster
        book.setBorrower(original.getBorrower());   // NEW CHALLENGE: keep the borrower!
        book.setCreatedAt(original.getCreatedAt()); // keep the original timestamp
